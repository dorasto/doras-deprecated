        ...
        <section className="mx-auto text-surface-800">
            <div className="mb-4 grid grid-cols-12 gap-4">
                ...
                <BounceCard href="#features" className="col-span-12 md:col-span-4 row-span-2 bg-gradient-to-tl from-violet-400 via-pink-300 to-lime-300 shadow-lg">
                    ...
                    <div className="absolute  bottom-0 left-4 right-4 top-20  translate-y-8 scale-75 rounded-t-2xl bg-transparent p-4 transition-transform duration-700 group-hover:translate-y-4 group-hover:rotate-[2deg]">
...
export const BounceCard = ({ className, children, href, whileHover = { scale: 0.95, rotate: "-1deg" } }: { className: string; href?: string; children: ReactNode; whileHover?: any }) => {
    return (
        <motion.a href={href} whileHover={whileHover !== "false" ? whileHover : undefined} className={`group relative min-h-[300px] cursor-pointer overflow-hidden rounded-2xl bg-slate-100 p-8 z-auto ${className}`}>
            {children}
        </motion.a>
    );
};
...
export const CardTitle = ({ children, className }: { children: ReactNode; className?: string }) => {
    return <h3 className={`relative z-50 mx-auto text-center text-4xl font-black ${className}`}>{children}</h3>;
};

export const CardDescription = ({ children, className }: { children: ReactNode; className?: string }) => {
    return <h3 className={`relative text-center text-xl font-semibold max-w-md z-50 ${className}`}>{children}</h3>;
};
